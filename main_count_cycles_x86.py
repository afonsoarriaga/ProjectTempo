...
def print_table(row_labels, col_labels, values):
    ...
    # convert all cells to formatted strings
    ...
    for c in range(ncols):
        w = len(col_labels[c])
        for r in range(nrows):
            w = max(w, len(cells[r][c]))
        col_widths.append(w + 2)

    row_label_width = max(20, max(len(lbl) for lbl in row_labels) + 2)

    # print header
    header = "".ljust(row_label_width)
    for c in range(ncols):
        header += " | " + col_labels[c].center(col_widths[c])
    print("\n" + header)
    ...
    for r, label in enumerate(row_labels):
        line = label.ljust(row_label_width)
        for c in range(ncols):
            line += " | " + cells[r][c].rjust(col_widths[c])
        print(line)
    print()
...
def bench_gen_matrix(gen_matrix, k: int):
    ...
    rsd = (sd / avg * 100) if avg > 0 else 0.0
    return avg, rsd

def bench_kem(keypair, enc, dec, k_idx: int):
    pk_len = PK_SIZES[k_idx]
    sk_len = SK_SIZES[k_idx]
    ct_len = CT_SIZES[k_idx]
...
def main():
    ...
    samplentt_timings = [[(0.0, 0.0)] * NUM_ALGOS for _ in range(NUM_K)]
    ...
    for k_idx, k in enumerate(k_values):
        for a in range(NUM_ALGOS):
            ...
            samplentt_timings[k_idx][a] = (avg, rsd)
            print(f"... {kem_labels_gen[k_idx]} / {algo_names[a]}: {avg:.0f} ± {rsd:.2f}%")
    ...
    for k_idx in range(NUM_K):
        for a in range(NUM_ALGOS):
            lib = C.CDLL(LIB_PATHS[k_idx][a])
            keypair, enc, dec = resolve_kyber_api(lib, k_idx)
            avg, rsd = bench_kem(keypair, enc, dec, k_idx)
            kem_timings[k_idx][a] = (avg, rsd)
            print(f"... {kem_labels[k_idx]} / {algo_names[a]}: {avg:.0f} ± {rsd:.2f}%")
    ...
    for k_idx in range(NUM_K):
        for a in range(NUM_ALGOS):
            lib = C.CDLL(LIB_PATHS[k_idx][a])
            init_start, init_end, resp = resolve_pake_api(lib)
            avg, rsd = bench_pake(init_start, init_end, resp, k_idx)
            pake_timings[k_idx][a] = (avg, rsd)
            print(f"... {pake_labels[k_idx]} / {algo_names[a]}: {avg:.0f} ± {rsd:.2f}%")
    ...
    print("\nKEM results (keygen+enc+dec, cycles per run with RSD%)\n")
    print_table(kem_labels, algo_names, kem_timings)

    print("\nPAKE results (NoIC initStart+resp+initEnd, cycles per run with RSD%)\n")
