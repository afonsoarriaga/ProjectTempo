...
def print_table(row_labels, col_labels, values):
    ...
    for name in col_labels:
        pad = (15 - len(name)) // 2
        cell = " " * pad + name + " " * (15 - pad - len(name))
        print(" | " + cell, end="")
    print()
    print("-" * (18 + len(col_labels) * (15 + 3)))
    for r, label in enumerate(row_labels):
        print("%-18s" % label, end="")
        for c in range(len(col_labels)):
            print(" | %*.0f cycles" % (8, values[r][c]), end="")
        print()
    print()
...
def main():
    ...
    for k_idx, k in enumerate(k_values):
        for a in range(NUM_ALGOS):
            ...
            print(f"... {kem_labels_gen[k_idx]} / {algo_names[a]}: {t:.0f} cycles")
    ...
    for k_idx in range(NUM_K):
        for a in range(NUM_ALGOS):
            ...
            print(f"... {kem_labels[k_idx]} / {algo_names[a]}: {t:.0f} cycles")
    ...
    for k_idx in range(NUM_K):
        for a in range(NUM_ALGOS):
            ...
            print(f"... {pake_labels[k_idx]} / {algo_names[a]}: {t:.0f} cycles")

    # 3) Print tables
    print("\nSampleNTT results (gen_matrix; average cycles per run)\n")
    ...
    print("\nKEM results (keygen+enc+dec, average cycles per run)\n")
    print_table(kem_labels, algo_names, kem_timings)

    print("\nPAKE results (NoIC initStart+resp+initEnd, average cycles per run)\n")
